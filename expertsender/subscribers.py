...
def subscribe(api_key, email, list_id, fields, custom_fields=None, mode=SUBSCRIBE_ADD_AND_UPDATE):
    ...
    return requests.post('%s/Api/Subscribers/' % API_SERVER, data=xml, headers=headers, timeout=TIMEOUT)
...
def unsubscribe(api_key, email, list_id=None):
    ...
    return requests.delete('%s/Api/Subscribers?%s' % (API_SERVER, params), timeout=TIMEOUT)
